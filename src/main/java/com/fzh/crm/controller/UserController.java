...
import com.fzh.crm.controller.dto.EmailDTO;
...
public class UserController {
    ...
    @PostMapping("/login2")
    public Result login2(@RequestBody EmailDTO emailDTO) throws IOException {
        String email = emailDTO.getEmail();
        //获取缓存中的验证码
        EmailIdentify identify = new EmailIdentify();
        Boolean aBoolean = identify.validateCaptcha(email, emailDTO.getIdentify());
        if (StrUtil.isBlank(email) || !aBoolean) {
    ...
    @PostMapping("/getIdentify")
    public Result getIdentify(@RequestBody String email) throws IOException {
        ...
        if (m.matches()) {
            ...
            if (exist == null) {
                //自动注册用户
                boolean b = userService.addUser(email);
                if (b) {
                    String identify = userService.sendEmail(email);
                    return Result.success(identify);
                } else {
                    return Result.error("500", "自动注册新用户失败，请重新尝试");
                }
            }
            String identify = userService.sendEmail(email);
            return Result.success(identify);
